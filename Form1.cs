...
namespace MenuKhachHang
{
    public partial class Form1 : Form
    {
        ...
        //chuoiketnoi
        string connectionString = "Data Source=LINH\\MSSQLSERVER01;Initial Catalog=QuanLiCuaHang;Integrated Security=True;Trust Server Certificate=True";
        ...
        private void btnCheckout_Click(object sender, EventArgs e)
        {
            ...
            using (var frm = new FrmThanhToan(items, cartTotal))
            {
                var rs = frm.ShowDialog(this);
                if (rs == DialogResult.OK)
                {
                    // Người dùng ấn Xong ở trang Thanh toán -> dọn giỏ
        ...
        // ==== TẠO DANH SÁCH MÓN ĐỂ GỬI SANG FORM THANH TOÁN ====
        private List<FrmThanhToan.CheckoutItem> BuildCheckoutItems()
        {
            var result = new List<FrmThanhToan.CheckoutItem>();

            // itemQty: Tên món -> Số lượng (đã được cập nhật khi bấm Thêm vào giỏ)
            foreach (var kv in itemQty)
            {
                string tenMon = kv.Key;
                int soLuong = kv.Value;
                decimal donGia = FindPriceForItem(tenMon); // tìm đơn giá theo tên từ các dòng trong giỏ

                result.Add(new FrmThanhToan.CheckoutItem
                {
                    TenMon = tenMon,
                    SoLuong = soLuong,
                    DonGia = donGia
            ...
            // Phòng hờ: nếu vì lý do gì itemQty trống, đọc trực tiếp từ các dòng control trong flpCart
            if (result.Count == 0)
            {
                foreach (Control c in flpCart.Controls)
                {
                    if (c is CartItemControl row)
                    {
                        result.Add(new FrmThanhToan.CheckoutItem
                        {
                            TenMon = row.ItemName,
        ...
        // ==== TÌM GIÁ THEO TÊN MÓN TỪ CÁC DÒNG TRONG GIỎ ====
        private decimal FindPriceForItem(string tenMon)
        {
            foreach (Control c in flpCart.Controls)
            {
                if (c is CartItemControl row &&
                    string.Equals(row.ItemName, tenMon, StringComparison.CurrentCultureIgnoreCase))
                {
                    return row.Price;
                }
            }
